...
function validateAndFilterCandidates(rawCandidates, startDate, endDate) {
  ...
  for (const item of rawCandidates) {
    ...
    const publishedAtUtc = String(item.published_at_utc || "").trim();
    const dateEvidence = String(item.date_evidence || "").trim();
    const sourceType = String(item.source_type || "").trim();

    if (!company || !title || !url || !publishedAtUtc || !dateEvidence) {
      continue;
    }
    ...
    if (shouldExcludeByCompany(company)) {
      continue;
    }

    if (!isWithinPeriod(publishedAtUtc, startDate, endDate)) {
      continue;
    }

    if (item.is_within_period !== true) {
      continue;
    }
    ...
    validItems.push({
      ...
      importance: String(item.importance || "低").trim(),
      published_at_utc: new Date(publishedAtUtc).toISOString(),
      date_evidence: dateEvidence,
      url,
      source_type: sourceType || "不明",
      what_happened: String(item.what_happened || "").trim(),
      why_it_matters: String(item.why_it_matters || "").trim(),
      japan_market_impact: item.japan_market_impact
        ? String(item.japan_market_impact).trim()
        : "",
      japan_market_impact_reason: item.japan_market_impact_reason
        ? String(item.japan_market_impact_reason).trim()
        : "",
      linkedin_scope: item.linkedin_scope
        ? String(item.linkedin_scope).trim()
        : "",
      competition_with_udemy_business: item.competition_with_udemy_business
        ? String(item.competition_with_udemy_business).trim()
        : "",
    });
  }
...
このタスクでは、鮮度の誤りを最も重大な失敗とします。

対象期間内であることを、リンク先ページ本文内の発表日・公開日・更新日で確認できない情報は採用してはいけません。
  ...
  "candidates": [
    {
      ...
      "date_evidence": "リンク先ページ本文内で確認した日付の根拠。例：ページ本文に「2026年5月8日公開」と記載",
...
async function main() {
  ...
  if (verifiedItems.length === 0) {
    console.log("No verified candidates found within the target period.");
  }
