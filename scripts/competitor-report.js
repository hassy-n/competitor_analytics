...
const TARGETS = [
  {
    ...
    sourceHints: [
      ...
      "site:jp.linkedin.com/learning 日本語 LinkedIn Learning AI",
...
このステップでは、最終採用を判断しすぎないでください。

対象期間内の可能性がある候補は、根拠が弱くても候補として出してください。
根拠が弱いものは evidence_rank を C にしてください。
採用可否は後続のJS側で判断します。

つまり、このステップの役割は「候補を見つけること」です。
「迷う候補を捨てること」ではありません。
...
■ 採用範囲の目安
--------------------------------------------
候補として優先したいのは、以下に示唆があるものです。

${target.adoptionScope}

ただし、このステップでは最終採用を決めません。
少しでも対象期間内の可能性があり、業務示唆がありそうなものは候補に含めてください。
...
LinkedIn Learningのコースやコンテンツについては、日本語対応が確認できるものを最終採用したいです。

ただし、この候補抽出ステップでは、日本語対応が未確認のものも候補として出してよいです。
その場合は japanese_availability を「未確認」にしてください。
...
候補に含める場合は、japanese_availability と japanese_availability_evidence を記入してください。
日本語対応が確認できない場合も、対象期間内の有力候補であれば候補に含めてよいです。
その場合、japanese_availability は「未確認」とし、japanese_availability_evidence には「公式ページ上で日本語対応を確認できなかった」など、確認状況を正直に書いてください。
...
primary_url は個別ページを優先してください。
ただし、個別ページが見つからないが、公式一覧ページにタイトル・公開日・内容の一部が確認できる場合は、一覧ページを primary_url にしてもよいです。
その場合 evidence_rank は B または C にしてください。

トップページだけで内容の個別確認ができない場合は、evidence_rank を C にしてください。
...
注意：
検索結果の日付だけが新しい情報は、AまたはBにしてはいけません。
ただし、候補として気になる場合は C として出してよいです。
...
または、内容・日付・日本語対応などの一部が未確認で、最終採用には追加確認が必要。
この場合は evidence_rank を C にしてください。
